...
void handlePageMusic(){
	...
	if (musicOn && gameState != -1){
		if (gameState == 0 && isPhase1){
			PlaySound("music\\Sekiro_startGame.wav", NULL, SND_LOOP | SND_ASYNC);			//START GAME PAGE MUSIC		
		}
		else if (gameState == 0 && isPhase2){
			PlaySound("music\\phase2.wav", NULL, SND_LOOP | SND_ASYNC);			//START GAME PAGE MUSIC		
		}
		else if (gameState == 1){
		...
		else if (musicOn && gameState == 6){
			
			PlaySound("music\\Game Over Sound Effect.wav", NULL, SND_LOOP | SND_ASYNC);
		}
	}
...
void loadImage(){
	...
	if (gameState == -1){
		...
		iShowBMP(0, 0, characterBG[characterBGIndex]);
		iShowBMP2(0, 0, continuePopUp[continuePopUpIndex], 0);
		iText(200, 629, scoreText, GLUT_BITMAP_TIMES_ROMAN_24);
	}
...
void iMouseFunction_HomePage(int button, int state, int mx, int my)
{
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
	{
		...
		else if (gameState != -1 && gameState != 0 && (mx >= 10 && mx <= 60) && (my >= 660 && my <= 720) && !showExitPopup && !gamePaused)				// Upper Back Button Click Function
		{
			if (OPTIONS && gameState == 4){
				...
				if (gameOverScoreButtonClick && gameStart){								// backbutton x game over score x score from game over score
					gameState = 6;
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN){
		...
		if (gameState == 9){
			if (mx >= 704 && mx <= 876 && my >= 229 && my <= 284){		// yes click
				gameState = 0;
			...
			else if (mx >= 404 && mx <= 576 && my >= 229 && my <= 284){		// no click
				gameState = 6;
				if (!musicOn){
					playButtonClickSound();
				}
...
void iPassiveMouseMoveFunction_HomePage(int mx, int my){
	...
	if (gameState == 9){
		if (mx >= 704 && mx <= 876 && my >= 229 && my <= 284){		// yes over
			continuePopUpIndex = 1;
			gamePaused = false;
		}
		else if (mx >= 404 && mx <= 576 && my >= 229 && my <= 284){		// no over
			continuePopUpIndex = 2;
			gamePaused = false;
		}
